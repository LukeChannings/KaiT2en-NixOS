...
export async function attachTouchBar(): Promise<void> {
  ...
  // until reboot. This is a userspace-side guard independent of the kernel's
  // own stateful-resume fixes (EP0 stall recovery, resume ordering) — those
  // stop the *kernel* from desyncing itself, they don't stop us poking the
  // bus unnecessarily from here.
  //
